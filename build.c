...
#define SILENCE_WARNINGS "-Wno-attributes", "-Wno-cpp", "-Wno-unused-parameter"

int main(int argc, char *argv[]) {
  go_rebuild_urself(argc, argv, "libs/lbstd.h");
  ...
  while (1) {
    ...
    static struct option long_options[] = {
        {"new", no_argument, 0, 'n'},  {"run", no_argument, 0, 'r'},
        {"bear", no_argument, 0, 'b'}, {"Yes", no_argument, 0, 'y'},
        {"help", no_argument, 0, 'h'}, {0, 0, 0, 0}};

    opt = getopt_long(argc, argv, "+nrbyh", long_options, &option_index);
    if (opt == -1)
      break;
  ...
  if (needs_rebuild("orchestrator", (const char *[]){"orchestrator.c"}, 1)) {
    cmd_append(&c, __BEAR "cc", "-Wall", SILENCE_WARNINGS, "-Wextra", "-o",
               "orchestrator", "orchestrator.c", SANITIZERS);
