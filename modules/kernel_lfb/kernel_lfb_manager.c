...
EXPORT_RESULT_STRUCT_SIZE() { return sizeof(kernel_lfb_result_t); }

EXPORT_RESULT_STRUCT_DIAGNOSTICS(kernel_lfb_result_t *result) {
  plog(INFO, "kernel_lfb module called!");
  ...
  bool present =
      !are_close(cache_line_access_time, result->uncached_access_time, 40.f) &&
      cache_line_access_time < result->uncached_access_time;

  if (!present) {
    plog(
        INFO,
        "Speculative memory access not present to kernel memory. Meltdown like "
